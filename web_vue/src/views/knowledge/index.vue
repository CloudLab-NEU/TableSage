...
const handleQuestionClick = async (item: PageItem) => {
  ...
  try {
    ...
    const response = await api.get('/knowledge/similarity-graph', {
      params: {
...
const fetchPageData = async (page = 1, size = 7, keyword = '') => {
  ...
  try {
    ...
    const res = await api.get<ApiResponse>('/knowledge/questions', { 
      params,
      timeout: 5000 
    })
...
const fetchQuestionDetail = async (tableId: string) => {
  ...
  try {
    ...
    const response = await api.get(`/knowledge/question/${tableId}`, {
      timeout: 10000
    })
...
const fetchData = async () => {
  try {
    const [strategyRes, overviewRes] = await Promise.all([
      api.get('/knowledge/strategy-statistics', { timeout: 5000 }),
      api.get('/knowledge/overview', { timeout: 5000 })
    ])
